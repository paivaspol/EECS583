...
/home/arquinn/valgrind/bin/cg_annotate $BM_MERGE --threshold=0 --show=Dr,Dw,D1mr,D1mw | grep /home | sed "s/,//g" | awk '{n=split($5,words,"/"); split(words[n], funct, ":"); if($1 + $2 > 0) print funct[1], funct[2], (($3 + $4)/($1 + $2))}' > $BM_NAME.data.results.unlinked
